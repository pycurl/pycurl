...
do_curl_setopt_mimepost(CurlObject *self, PyObject *obj)
{
    ...
    if (!PyObject_TypeCheck(obj, p_CurlMime_Type)) {
        PyErr_SetString(PyExc_TypeError, "setopt(option=MIMEPOST) expects a Mime object");
        return NULL;
    }
    ...
    if (mime->curl != self) {
        PyErr_SetString(PyExc_ValueError, "setopt(option=MIMEPOST) requires a Mime created with this Curl handle");
        return NULL;
    }

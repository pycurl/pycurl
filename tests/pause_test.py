...
def _run_with_unpause(curl, state, resume_after, timeout):
...
def _run_download_pause(
    app, curl, *, mask=pycurl.PAUSE_ALL, resume_after=0.7, timeout=4.0
):
    ...
    start = time.monotonic()
    done_at, err_list = _run_with_unpause(curl, state, resume_after, timeout)
    return start, done_at, err_list, sio, state


def _run_upload_pause(
    app, curl, *, mask=pycurl.PAUSE_SEND, resume_after=0.2, timeout=6.0
):
    data = b"a" * 16384
    ...
    _, err_list = _run_with_unpause(curl, state, resume_after, timeout)
...
def test_pause_recv_mask(app, curl, mask, resume_after, min_wait):
    """Pause receiving via mask and ensure data resumes after unpause."""
    start, done_at, err_list, sio, state = _run_download_pause(
        app, curl, mask=mask, resume_after=resume_after, timeout=4.0
    )
...
def test_pause_via_write_return_replays_data(app, curl):
    ...
    _, err_list = _run_with_unpause(curl, state, resume_after=0.2, timeout=3.0)
...
def test_pause_via_read_return(app, curl):
    ...
    _, err_list = _run_with_unpause(curl, state, resume_after=0.2, timeout=4.0)
...
def test_pause_excludes_low_speed_limit_and_resets_timer(app, curl):
    ...
    done_at, err_list = _run_with_unpause(curl, state, resume_after=1.5, timeout=8.0)
